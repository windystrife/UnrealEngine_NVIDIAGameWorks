...
class Arm():
    ...
    def createElbowSwitch(self):
        ...
        # Create a new skeleton from clav to elbow for the elbow switch.
        elbowswitch_clav = cmds.duplicate("driver_clavicle"+self.suffix, po=True, n="elbowswitch_clavicle"+self.suffix)[0]
        cmds.parent(elbowswitch_clav, "ik_elbow"+self.suffix+"_anim_grp")
        elbowswitch_upperarm = cmds.duplicate("driver_upperarm"+self.suffix, po=True, n="elbowswitch_upperarm"+self.suffix)[0]
        cmds.parent(elbowswitch_upperarm, elbowswitch_clav)
        elbowswitch_lowerarm = cmds.duplicate("driver_lowerarm"+self.suffix, po=True, n="elbowswitch_lowerarm"+self.suffix)[0]
        cmds.parent(elbowswitch_lowerarm, elbowswitch_upperarm)

        # Create parent constraints to make the new elbowswitch joints follow the driver joints.
        cmds.parentConstraint("driver_clavicle"+self.suffix, elbowswitch_clav, mo=True)
        cmds.parentConstraint("driver_upperarm"+self.suffix, elbowswitch_upperarm, mo=True)
        cmds.parentConstraint("driver_lowerarm"+self.suffix, elbowswitch_lowerarm, mo=True)
        ...
        cmds.makeIdentity(elbowswitch, a=True, t=True, r=True, s=True)

        cmds.parent(elbowswitch, elbowswitch_lowerarm)
        cmds.setAttr(elbowswitch_clav + ".v", 0)
