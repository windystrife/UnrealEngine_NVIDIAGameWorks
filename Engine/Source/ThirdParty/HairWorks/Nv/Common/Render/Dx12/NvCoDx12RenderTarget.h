...
#include <Nv/Common/Render/Dx12/NvCoDx12DescriptorHeap.h>
...
class Dx12RenderTarget
{
	...
	Result init(RenderContext* renderContext, const Desc& desc);
	...
	void bindAndClear(RenderContext* renderContext);
	
	void toWritable(RenderContext* renderContext);
	void toReadable(RenderContext* renderContext);
		
		/// Set the debug name
	void setDebugName(BufferType type, const SubString& name);
	...
	Int allocateSrvView(BufferType type, ID3D12Device* device, Dx12DescriptorHeap& heap, const D3D12_SHADER_RESOURCE_VIEW_DESC* desc = NV_NULL);
	...
	NV_FORCE_INLINE Int getNumRenderTargets() const { return m_renderTarget ? 1 : 0; }
	...
	NV_FORCE_INLINE Int getSrvHeapIndex(BufferType type) const { return m_srvIndices[type]; }
