...
class Dx12ResourceScopeManager
{
	NV_CO_DECLARE_CLASS_BASE(Dx12ResourceScopeManager);
		...
		/// Initialize
	Result init(ID3D12Device* device, Dx12CounterFence* fence);
	
		/// Allocate constant buffer of specified size
	ID3D12Resource* newUploadResource(const D3D12_RESOURCE_DESC& resourceDesc, const D3D12_CLEAR_VALUE* clearValue = NV_NULL);
		/// Do an upload on the specified command list
	Result upload(ID3D12GraphicsCommandList* commandList, const Void* srcDataIn, ID3D12Resource* target, D3D12_RESOURCE_STATES targetState);
		/// Upload data to the target, and set state. Will add barriers if target is in the wrong state
	Result uploadWithState(ID3D12GraphicsCommandList* commandList, const Void* srcDataIn, Dx12Resource& target, D3D12_RESOURCE_STATES targetState);
	...
	static Void copy(const D3D12_SUBRESOURCE_DATA& src, SizeT rowSizeInBytes, Int numRows, Int numSlices, const D3D12_MEMCPY_DEST& dst);

	protected:
	
	struct Entry
	{
		UInt64 m_completedValue;		///< If less than fence value this is completed
		ID3D12Resource* m_resource;		///< The mapped resource
	};
	
	Queue<Entry> m_entryQueue;
