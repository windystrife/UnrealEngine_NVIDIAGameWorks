...
#include <Nv/Common/Platform/Win/NvCoWinStringUtil.h>
...
/* static */void Dx12Resource::setDebugName(ID3D12Resource* resource, const SubString& name)
{
	if (resource)
	{
		Array<wchar_t> buf;
		WinStringUtil::appendWideChars(name, buf);
		resource->SetName(buf.begin());
	}
}

Void Dx12Resource::setDebugName(const SubString& name)
{
...
Result Dx12Resource::initCommitted(ID3D12Device* device, const D3D12_HEAP_PROPERTIES& heapProps, D3D12_HEAP_FLAGS heapFlags, const D3D12_RESOURCE_DESC& resourceDesc, D3D12_RESOURCE_STATES initState, const D3D12_CLEAR_VALUE * clearValue)
{
	setResourceNull();
	NvCo::ComPtr<ID3D12Resource> resource;
	NV_RETURN_ON_FAIL(device->CreateCommittedResource(&heapProps, heapFlags, &resourceDesc, initState, clearValue, IID_PPV_ARGS(resource.writeRef())));
