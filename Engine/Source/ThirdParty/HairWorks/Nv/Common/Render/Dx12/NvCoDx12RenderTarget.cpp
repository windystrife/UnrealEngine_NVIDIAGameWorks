...
{
	Memory::set(m_srvIndices, -1, sizeof(m_srvIndices));
}

Result Dx12RenderTarget::init(RenderContext* renderContext, const Desc& desc)
{
	Dx12RenderInterface* appIntf = renderContext->getInterface<Dx12RenderInterface>();
	NV_CORE_ASSERT(appIntf);
	ID3D12Device* device = appIntf->getDx12Device();
	...
	if (desc.m_targetFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		NV_RETURN_ON_FAIL(device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(m_descriptorHeapRtv.writeRef())));
	...
	if (desc.m_depthStencilFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		heapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
		NV_RETURN_ON_FAIL(device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(m_descriptorHeapDsv.writeRef())));
...
void Dx12RenderTarget::bind(RenderContext* renderContext)
{
	Dx12RenderInterface* intf = renderContext->getInterface<Dx12RenderInterface>();
	NV_CORE_ASSERT(intf);
	ID3D12GraphicsCommandList* commandList = intf->getDx12CommandList();
	...
	{
		commandList->OMSetRenderTargets(numTargets, &rtvHandle, FALSE, NV_NULL);
	}
...
void Dx12RenderTarget::clear(RenderContext* renderContext)
{
	Dx12RenderInterface* intf = renderContext->getInterface<Dx12RenderInterface>();
	NV_CORE_ASSERT(intf);
	ID3D12GraphicsCommandList* commandList = intf->getDx12CommandList();
	...
	if (m_renderTarget)
	{
		...
		commandList->ClearRenderTargetView(rtvHandle, &m_desc.m_targetClearColor.x, 1, &rect);
	}
	if (m_depthStencilBuffer)
	{
...
void Dx12RenderTarget::bindAndClear(RenderContext* renderContext)
{
	Dx12RenderInterface* intf = renderContext->getInterface<Dx12RenderInterface>();
	NV_CORE_ASSERT(intf);
	ID3D12GraphicsCommandList* commandList = intf->getDx12CommandList();
...
void Dx12RenderTarget::toWritable(RenderContext* renderContext)
{
	Dx12RenderInterface* rendIntf = renderContext->getInterface<Dx12RenderInterface>();
	Dx12BarrierSubmitter submitter(rendIntf->getDx12CommandList());
...
void Dx12RenderTarget::toReadable(RenderContext* renderContext)
{
	Dx12RenderInterface* rendIntf = renderContext->getInterface<Dx12RenderInterface>();
	Dx12BarrierSubmitter submitter(rendIntf->getDx12CommandList());
...
Int Dx12RenderTarget::allocateSrvView(BufferType type, ID3D12Device* device, Dx12DescriptorHeap& heap, const D3D12_SHADER_RESOURCE_VIEW_DESC* desc)
{
	...
	{
		NV_CORE_ALWAYS_ASSERT("A srv index has already been associated!");
...
void Dx12RenderTarget::setDebugName(BufferType type, const SubString& name)
{
	getResource(type).setDebugName(name);
}

void Dx12RenderTarget::setDebugName(const SubString& name)
{
	String buf(name);
	buf << " [Target]";
