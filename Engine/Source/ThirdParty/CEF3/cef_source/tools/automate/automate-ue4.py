...
def make_directory_rw(path):
  """ copy_directory copies all permissions, which can cause problems later on, so call this function to add write permissions to the destination """
  if options.dryrun:
    return
  for root, dirs, files in os.walk(path):
    for dir in dirs:
      ...
      newmode = 0o755
      print "%s: %o, %o" % (dirpath, mode, newmode)
      os.chmod(dirpath, newmode )
    for file in files:
      filepath = os.path.join(root, file)
      mode = os.stat(filepath).st_mode
      newmode = 0o755 if mode & stat.S_IEXEC else 0o644
      print "%s: %o, %o" % (filepath, mode, newmode)
      if newmode != mode:
...
distribution of CEF from a static source directory.
"""
...
if options.clientdistrib or options.clientdistribonly:
  ...
  if options.buildtarget.find(client_app) == -1:
    ...
    sys.exit()
# Set cef_dir early
cef_dir = os.path.abspath(os.path.join(script_dir, '../..'))

# CEF branch - read it from our VENDOR_BRANCH text file
revision_path = os.path.join(cef_dir, 'UE_VENDOR_REVISION.txt')
revision_config = read_config_file(revision_path)

if 'branch' in revision_config:
	cef_branch = revision_config['branch']
else:
	raise Exception("Missing branch value in %s" % (revision_path))

# True if the requested branch is 2272 or newer.
branch_is_2272_or_newer = int(revision_config['branch']) >= 2272

# True if the requested branch is 2357 or newer.
branch_is_2357_or_newer = int(revision_config['branch']) >= 2357

# True if the requested branch is 2743 or older.
branch_is_2743_or_older = int(revision_config['branch']) <= 2743

# True if the requested branch is newer than 2785.
branch_is_newer_than_2785 = int(revision_config['branch']) > 2785
...
# Determine the CEF Git branch we're using.
...
# This will control wether we will perform a rebuild or not
cef_checkout_changed = True
...
if not options.nochromiumupdate and not os.path.exists(chromium_src_dir):
  ...
  run("gclient sync --nohooks --with_branch_heads --jobs 8", chromium_dir, \
      depot_tools_dir)
else:
...
if chromium_checkout_changed:
  ...
  run("gclient sync %s--with_branch_heads --jobs 8" % \
