...
public:
	...
	*/
	virtual int32_t GetD3D9AdapterIndex() = 0; // DEPRECATED
	...
	*/
	virtual void GetDXGIOutputInfo( int32_t *pnAdapterIndex ) = 0; // DEPRECATED

	/**
	 * Returns platform-specific adapter identification so that applications and the compositor are
	 * creating textures and swap chains on the same GPU. If an error occurs the device will be
	 * set to 0.
	 * [D3D10/11/12 Only (D3D9 Not Supporged)]
	 ...
	 */
	virtual void GetOutputDevice( uint64_t *pnDevice ) = 0;
	...
	*/
	...
	public:
		...
		IVRSystem *VRSystem()
		{
			...
			{
				EVRInitError eError;
				m_pVRSystem = ( IVRSystem * )m_pVRGenericInterface( IVRSystem_Version, &eError );
			}
			return m_pVRSystem;
		}
		IVRChaperone *VRChaperone()
		{
			...
			{
				EVRInitError eError;
				m_pVRChaperone = ( IVRChaperone * )m_pVRGenericInterface( IVRChaperone_Version, &eError );
			}
		...
		IVRChaperoneSetup *VRChaperoneSetup()
		{
			...
			{
				EVRInitError eError;
				m_pVRChaperoneSetup = ( IVRChaperoneSetup * )m_pVRGenericInterface( IVRChaperoneSetup_Version, &eError );
			}
		...
		IVRCompositor *VRCompositor()
		{
			...
			{
				EVRInitError eError;
				m_pVRCompositor = ( IVRCompositor * )m_pVRGenericInterface( IVRCompositor_Version, &eError );
			}
		...
		IVROverlay *VROverlay()
		{
			...
			{
				EVRInitError eError;
				m_pVROverlay = ( IVROverlay * )m_pVRGenericInterface( IVROverlay_Version, &eError );
			}
			return m_pVROverlay;
		}
		...
		IVRResources *VRResources()
		{
			...
			{
				EVRInitError eError;
				m_pVRResources = (IVRResources *)m_pVRGenericInterface( IVRResources_Version, &eError );
			}
		...
		IVRScreenshots *VRScreenshots()
		{
			...
			{
				EVRInitError eError;
				m_pVRScreenshots = ( IVRScreenshots * )m_pVRGenericInterface( IVRScreenshots_Version, &eError );
			}
		...
		IVRRenderModels *VRRenderModels()
		{
			...
			{
				EVRInitError eError;
				m_pVRRenderModels = ( IVRRenderModels * )m_pVRGenericInterface( IVRRenderModels_Version, &eError );
			}
		...
		IVRExtendedDisplay *VRExtendedDisplay()
		{
			...
			{
				EVRInitError eError;
				m_pVRExtendedDisplay = ( IVRExtendedDisplay * )m_pVRGenericInterface( IVRExtendedDisplay_Version, &eError );
			}
		...
		IVRSettings *VRSettings()
		{
			...
			{
				EVRInitError eError;
				m_pVRSettings = ( IVRSettings * )m_pVRGenericInterface( IVRSettings_Version, &eError );
			}
		...
		IVRApplications *VRApplications()
		{
			...
			{
				EVRInitError eError;
				m_pVRApplications = ( IVRApplications * )m_pVRGenericInterface( IVRApplications_Version, &eError );
			}
		...
		IVRTrackedCamera *VRTrackedCamera()
		{
			...
			{
				EVRInitError eError;
				m_pVRTrackedCamera = ( IVRTrackedCamera * )m_pVRGenericInterface( IVRTrackedCamera_Version, &eError );
			}
