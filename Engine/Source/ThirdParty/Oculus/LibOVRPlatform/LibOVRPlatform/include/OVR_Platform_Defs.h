...
//
// OVRP_PUBLIC_FUNCTION  - Functions that externally visible from a shared library. Corresponds to Microsoft __dllexport.
// OVRP_PUBLIC_CLASS     - C++ structs and classes that are externally visible from a shared library. Corresponds to Microsoft __dllexport.
//
// OVRP_DLL_BUILD        - Used to indicate that the current compilation unit is of a shared library.
// OVRP_DLL_IMPORT       - Used to indicate that the current compilation unit is a user of the corresponding shared library (default)
// OVRP_DLL_BUILD        - used to indicate that the current compilation unit is not a shared library but rather statically linked code.
//
        ...
        #else
            #define OVRP_PUBLIC_FUNCTION(rval) OVRP_EXTERN_C __attribute__((visibility("default"))) rval OVRP_CDECL /* Requires GCC 4.0+ */
