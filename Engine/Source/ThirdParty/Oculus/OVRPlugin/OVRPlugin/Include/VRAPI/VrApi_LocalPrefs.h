...
// For Snapdragon Profiler to capture, we need to issue eglSwapBuffers even if we are
// rendering to the front buffer.
#define LOCAL_PREF_VRAPI_FORCE_SWAPS					"dev_forceSwaps"				// "0" = no swaps, "1" = swap on vr thread, "2" = swap on time warp thread, "3" = both
