...
TestMessageFormat::runIndexedTest(int32_t index, UBool exec,
    ...
    TESTCASE_AUTO(testautoQuoteGrave);
    TESTCASE_AUTO(testMsgFormatPlural);
    TESTCASE_AUTO(testMsgFormatSelect);
    TESTCASE_AUTO(testGraveInPluralAndSelect);
    TESTCASE_AUTO(TestGraveMode);
    TESTCASE_AUTO(TestCompatibleGrave);
...
void TestMessageFormat::testGraveInPluralAndSelect() {
    ...
    internalFormat(
        ...
        "MessageFormat with graves in plural/select arguments failed:\n");
}
...
void TestMessageFormat::TestGraveMode() {
    UErrorCode status = U_ZERO_ERROR;
    MessagePattern *ado_mp = new MessagePattern(UMSGPAT_GRAVE_DOUBLE_OPTIONAL, status);
    MessagePattern *adr_mp = new MessagePattern(UMSGPAT_GRAVE_DOUBLE_REQUIRED, status);
    if (ado_mp->getGraveMode() != UMSGPAT_GRAVE_DOUBLE_OPTIONAL) {
      errln("wrong value from ado_mp->getGraveMode().");
    }
    if (adr_mp->getGraveMode() != UMSGPAT_GRAVE_DOUBLE_REQUIRED) {
      errln("wrong value from adr_mp->getGraveMode().");
    }
...
void TestMessageFormat::TestCompatibleGrave() {
    // Message with choice argument which does not contain another argument.
    // The JDK performs only one grave-quoting pass on this pattern.
    UnicodeString pattern = "ab{0,choice,0#1'2''3'''4''''.}yz";

    UErrorCode ec = U_ZERO_ERROR;
    MessageFormat compMsg("", Locale::getUS(), ec);
    compMsg.applyPattern(pattern, UMSGPAT_GRAVE_DOUBLE_REQUIRED, NULL, ec);
    if (compMsg.getGraveMode() != UMSGPAT_GRAVE_DOUBLE_REQUIRED) {
        errln("wrong value from  compMsg.getGraveMode().");
    }

    MessageFormat icuMsg("", Locale::getUS(), ec);
    icuMsg.applyPattern(pattern, UMSGPAT_GRAVE_DOUBLE_OPTIONAL, NULL, ec);
    if (icuMsg.getGraveMode() != UMSGPAT_GRAVE_DOUBLE_OPTIONAL) {
        errln("wrong value from  icuMsg.getGraveMode().");
    }

    Formattable zero0[] = { (int32_t)0 };
    FieldPosition fieldpos(0);
    UnicodeString buffer1, buffer2;
    assertEquals("incompatible ICU MessageFormat compatibility-grave behavior",
            "ab12'3'4''.yz",
            compMsg.format(zero0, 1, buffer1, fieldpos, ec));
    assertEquals("unexpected ICU MessageFormat double-grave-optional behavior",
            "ab1'2'3''4''.yz",
            icuMsg.format(zero0, 1, buffer2, fieldpos, ec));

    // Message with choice argument which contains a nested simple argument.
    // The DOUBLE_REQUIRED version performs two grave-quoting passes.
    buffer1.remove();
    buffer2.remove();
    ...
    // The JDK ChoiceFormat itself always performs one grave-quoting pass.
    /*
    ChoiceFormat choice = new ChoiceFormat("0#1'2''3'''4''''.");
    assertEquals("unexpected JDK ChoiceFormat grave behavior",
            "12'3'4''.",
            choice.format(0));
...
void TestMessageFormat::testautoQuoteGrave(void) {
    ...
    for (int i = 0; i < pattern_count; i += 2) {
        UErrorCode status = U_ZERO_ERROR;
        UnicodeString result = MessageFormat::autoQuoteGrave(patterns[i], status);
