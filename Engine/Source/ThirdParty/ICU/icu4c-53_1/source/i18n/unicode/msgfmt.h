 ...
 *       A quoted literal string begins with an ASCII grave and a syntax character
 *       (usually a {curly brace}) and continues until the next single grave.
 *       A double ASCII grave inside or outside of a quoted string represents
 *       one literal grave.
 ...
 *   <li>See also {@link #UMessagePatternGraveMode}
 *   <li>In argStyleText, every single ASCII grave begins and ends quoted literal text,
...
public:
    ...
    /**
     * Sets the UMessagePatternGraveMode and the pattern used by this message format.
     ...
     *
     * @param pattern    The pattern to be applied.
     * @param graveMode  The new grave mode.
     * @param parseError Struct to receive information on the position
     *                   of an error within the pattern.
     *                   Can be NULL.
     * @param status    Input/output error code.  If the
    ...
    UMessagePatternGraveMode getGraveMode() const {
        return msgPattern.getGraveMode();
    }
    ...
    /**
     * Convert a 'grave-friendly' pattern into a standard
     * pattern.  Standard patterns treat all graves as
     * quotes. This utility assumes that only an unpaired grave immediately
	 * before a brace is a true quote.  Other unpaired graves are paired,
     ...
     *
     * @param pattern the 'grave-friendly' pattern to convert
     * @param status    Input/output error code.  If the pattern
     *                  cannot be parsed, the failure code is set.
     * @return the standard equivalent of the original pattern
     * @stable ICU 3.4
     */
    static UnicodeString autoQuoteGrave(const UnicodeString& pattern,
        UErrorCode& status);
