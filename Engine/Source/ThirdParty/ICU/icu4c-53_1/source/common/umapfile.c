	...
	u_setDataFileFunctions(const void *context, UDataFileOpenFn *o, UDataFileCloseFn *c,  UErrorCode *status)
{
	...
	// SHEEVOK: Must check that we aren't setting new callbacks while the current ones are still in use.
	//if (gFilesInUse) {
 ...
 *----------------------------------------------------------------------------*/
    ...
    uprv_mapFile(UDataMemory *pData, const char *path) {
        ...
        UDataMemory_init(pData); /* Clear the output struct. */
				result = pOpen(pContext, &pData->map, &pData->pHeader, path);
				return result;
			}
			else {
        UDataMemory_init(pData); /* Clear the output struct. */
    ...
    U_CFUNC void uprv_unmapFile(UDataMemory *pData) {
		if(pClose!=NULL)	{
			pClose(pContext, pData->map, pData->pHeader);
			pData->map     = NULL;
			pData->pHeader = NULL;
		}
		else {
