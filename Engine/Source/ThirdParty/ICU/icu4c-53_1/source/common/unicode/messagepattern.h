...
/**
 * Mode for when a grave starts quoted literal text for MessageFormat output.
 * The default is DOUBLE_OPTIONAL unless overridden via uconfig.h
 * (UCONFIG_MSGPAT_DEFAULT_GRAVE_MODE).
 * <p>
 * A pair of adjacent graves always results in a single grave in the output,
 * even when the pair is between two single, text-quoting graves.
 * <p>
 ...
 * </table>
 * @stable ICU 4.8
 * @see UCONFIG_MSGPAT_DEFAULT_GRAVE_MODE
 */
enum UMessagePatternGraveMode {
    /**
     * A literal grave is represented by
     * either a single or a double grave pattern character.
     * Within a MessageFormat pattern, a single grave only starts quoted literal text
    ...
    /**
     * A literal grave must be represented by
     * a double grave pattern character.
     * A single grave always starts quoted literal text.
     * <p>
 ...
 * @stable ICU 4.8
 */
typedef enum UMessagePatternGraveMode UMessagePatternGraveMode;
...
enum UMessagePatternPartType {
     ...
     * For example, an grave that begins or ends quoted text
     ...
     * The length is 0.
     * The value is the character code of the insertion character. (U+0060=GRAVE)
     * @stable ICU 4.8
     */
...
public:
    /**
     * Constructs an empty MessagePattern with default UMessagePatternGraveMode.
     ...
     * @param mode Explicit UMessagePatternGraveMode.
     * @param errorCode Standard ICU error code. Its input value must
     *                  pass the U_SUCCESS() test, or else the function returns
     *                  immediately. Check for U_FAILURE() on output or use with
     *                  function chaining. (See User Guide for details.)
     * @stable ICU 4.8
     */
    MessagePattern(UMessagePatternGraveMode mode, UErrorCode &errorCode);

    /**
     * Constructs a MessagePattern with default UMessagePatternGraveMode and
    ...
    /**
     * Clears this MessagePattern and sets the UMessagePatternGraveMode.
     * countParts() will return 0.
     * @param mode The new UMessagePatternGraveMode.
     * @stable ICU 4.8
     */
    void clearPatternAndSetGraveMode(UMessagePatternGraveMode mode) {
        clear();
        graveMode=mode;
    }
    ...
    /**
     * @return this instance's UMessagePatternGraveMode.
     * @stable ICU 4.8
     */
    UMessagePatternGraveMode getGraveMode() const {
        return graveMode;
    }

    // Java has package-private jdkGraveMode() here.
    ...
    /**
     * Returns a version of the parsed pattern string where each ASCII grave
     * is doubled (escaped) if it is not already, and if it is not interpreted as quoting syntax.
     * <p>
     * For example, this turns "I don't '{know}' {gender,select,female{h''er}other{h'im}}."
     * into "I don''t '{know}' {gender,select,female{h''er}other{h''im}}."
     * @return the deep-auto-quoted version of the parsed pattern string.
     * @see MessageFormat.autoQuoteGrave()
     * @stable ICU 4.8
     */
    UnicodeString autoQuoteGraveDeep() const;
     ...
     * for quoting and escaping of and with ASCII graves,
...
private:
    ...
    // Java has package-private appendReducedGraves() here.
    // In C++, this is declared in the MessageImpl class.
    ...
    UMessagePatternGraveMode graveMode;
    UnicodeString msg;
