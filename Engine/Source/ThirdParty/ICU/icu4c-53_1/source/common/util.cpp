...
static const UChar GRAVE      = 0x0060; // '`'
...
void ICU_Utility::appendToRule(UnicodeString& rule,
    ...
    if (isLiteral ||
        ...
        if (quoteBuf.length() > 0) {
            // We prefer backslash GRAVE to double GRAVE
            // (more readable, less similar to ") so if there are
            // double GRAVEs at the ends, we pull them outside
            // of the quote.

            // If the first thing in the quoteBuf is GRAVE
            // (doubled) then pull it out.
            while (quoteBuf.length() >= 2 &&
                   quoteBuf.charAt(0) == GRAVE &&
                   quoteBuf.charAt(1) == GRAVE) {
                rule.append(BACKSLASH).append(GRAVE);
                quoteBuf.remove(0, 2);
            }
            // If the last thing in the quoteBuf is GRAVE
            // (doubled) then remove and count it and add it after.
            int32_t trailingCount = 0;
            while (quoteBuf.length() >= 2 &&
                   quoteBuf.charAt(quoteBuf.length()-2) == GRAVE &&
                   quoteBuf.charAt(quoteBuf.length()-1) == GRAVE) {
