...
$out_def ="out"; $out_def.="32" if ($FLAVOR =~ /WIN32/); $out_def.="64" if ($FLAVOR =~ /WIN64/); $out_def.="CE" if ($FLAVOR =~ /CE/);	$out_def.="dll"			if ($shlib);
			$out_def.='_$(TARGETCPU)'	if ($FLAVOR =~ /CE/);
$tmp_def ="tmp"; $tmp_def.="32" if ($FLAVOR =~ /WIN32/); $tmp_def.="64" if ($FLAVOR =~ /WIN64/); $tmp_def.="CE" if ($FLAVOR =~ /CE/);	$tmp_def.="dll"			if ($shlib);
			$tmp_def.='_$(TARGETCPU)'	if ($FLAVOR =~ /CE/);
$inc_def="inc"; $inc_def.="32" if ($FLAVOR =~ /WIN32/); $inc_def.="64" if ($FLAVOR =~ /WIN64/); $inc_def.="CE" if ($FLAVOR =~ /CE/);
