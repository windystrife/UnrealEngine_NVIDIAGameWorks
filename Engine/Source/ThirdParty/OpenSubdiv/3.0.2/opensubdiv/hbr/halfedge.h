...
public:
    ...
    void SetOpposite(HbrHalfedge<T>* inopposite) { this->opposite = inopposite; sharpness = inopposite->sharpness; }
...
template <class T>
void
HbrHalfedge<T>::Initialize(HbrHalfedge<T>* inopposite, int index, HbrVertex<T>* origin,
    ...
    this->opposite = inopposite;
    incidentVertex = origin->GetID();
    lastedge = (index == face->GetNumVertices() - 1);
    firstedge = (index == 0);
    if (inopposite) {
        sharpness = inopposite->sharpness;
#ifdef HBRSTITCH
        StitchEdge **stitchEdges = face->stitchEdges +
            mesh->GetStitchCount() * index;
        for (int i = 0; i < mesh->GetStitchCount(); ++i) {
            stitchEdges[i] = inopposite->getStitchEdges()[i];
        }
        stitchccw = inopposite->stitchccw;
        raystitchccw = inopposite->raystitchccw;
        stitchdatavalid = 0;
        if (stitchEdges && inopposite->GetStitchData()) {
            mesh->SetStitchData(this, inopposite->GetStitchData());
            stitchdatavalid = 1;
        }
#endif
        if (fvarbits) {
            ...
            int fvarbitsSizePerEdge = ((fvarcount + 15) / 16);
            memcpy(fvarbits, inopposite->getFVarInfSharp(), fvarbitsSizePerEdge * sizeof(unsigned int));
        }
    } else {
        sharpness = 0.0f;
#ifdef HBRSTITCH
