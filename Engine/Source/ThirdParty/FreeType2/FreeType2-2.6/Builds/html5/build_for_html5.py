...
# This script builds freetype for HTML5 using Esmcripten.
# Usage:
...
for (mode, cmake_build_type) in build_modes:
	...
	# Run the build
	cmd = ['cmake', '--build', '.', '--', 'freetype', '-j']
	if 'verbose' in sys.argv: cmd += ['VERBOSE=1']
	run(cmd)

	# Deploy the output to the libraries directory.
	output_file = os.path.join(output_lib_directory, 'libfreetype260' + mode.replace('-', '_') + '.bc')
	print 'Deploying ' + output_file
	shutil.copyfile(os.path.join(build_dir, 'libfreetype.bc'), output_file)
