...
namespace Forsyth
{
    ...
    //-----------------------------------------------------------------------------
    void OptimizeFaces(const uint* indexList, uint indexCount, uint vertexCount, uint* newIndexList, uint16 lruCacheSize);
    ...
    void OptimizeFaces(const uint* indexList, uint indexCount, uint vertexCount, uint* newIndexList, uint16 lruCacheSize)
    {
        ...
        for (uint i = 0; i < indexCount; i += 3)
        {
            ...
            entriesInCache0 = std::min(entriesInCache1, (uint)lruCacheSize);
        }
    }
