...
namespace IncludeTool
{
	...
	class ManagedProcess : IDisposable
	{
		...
		const UInt32 JOB_OBJECT_LIMIT_PROCESS_MEMORY = 0x00000100;
		...
		[DllImport("kernel32.dll", SetLastError=true)]
		static extern int TerminateProcess(SafeFileHandle hProcess, uint uExitCode);

		[DllImport("kernel32.dll", SetLastError=true)]
		static extern uint WaitForSingleObject(SafeFileHandle hHandle, uint dwMilliseconds);
		...
		public bool TryReadAllLines(TimeSpan TimeLimit, List<string> OutputLines)
		{
		...
		/// <summary>
		/// Terminate the process and waits for it to exit
		/// </summary>
