...
#define PUTU32(ciphertext, st) { (ciphertext)[0] = (uint8)((st) >> 24); \
							  (ciphertext)[1] = (uint8)((st) >> 16); \
							  (ciphertext)[2] = (uint8)((st) >>  8); \
							  (ciphertext)[3] = (uint8)((st) >>  0); }
 ...
 * @return the number of rounds for the given cipher key size.
 */
static int32 rijndaelSetupDecrypt( uint32* rk, const uint8* key, int32 keybits )
{
	int32 nrounds, i, j;
	uint32 temp;
...
static void rijndaelEncrypt( const uint32 *rk, int32 nrounds, const uint8 plaintext[16], uint8 ciphertext[16] )
{
...
static void rijndaelDecrypt( const uint32 *rk, int32 nrounds, const uint8 ciphertext[16], uint8 plaintext[16] )
{
	uint32 s0, s1, s2, s3, t0, t1, t2, t3;
