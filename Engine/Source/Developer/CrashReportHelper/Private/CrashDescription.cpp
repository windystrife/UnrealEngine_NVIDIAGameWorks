...
{
	const bool bAddPersonalData = FCrashReportConfig::Get().GetAllowToBeContacted() || FEngineBuildSettings::IsInternalBuild();
	...
	const bool bSendUserName = FCrashReportConfig::Get().GetSendLogFile() || FEngineBuildSettings::IsInternalBuild();
	if (bSendUserName)
	{
...
FCrashContext::FCrashContext( const FString& CrashContextFilepath )
{
	...
	if (bIsValid)
	{
		...
		if (CrashDumpMode == ECrashDumpMode::FullDump)
		{
			...
			const FString LocationForBranch = FCrashReportConfig::Get().GetFullCrashDumpLocationForBranch( EngineVersion.GetBranch() );
